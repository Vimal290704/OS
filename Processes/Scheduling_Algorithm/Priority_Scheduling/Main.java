        ...
        while (true) {
            for (int i = 0; i < n; i++) {
                ...
                if (!processed[i] && arrival[i] <= currTime) {
                    ...
                    queue.add(new Process(arrival[i], burst[i], priority[i]));
                    processed[i] = true;
                }
            }
            ...
            if (queue.isEmpty()) {
                ...
                queue.add(new Process(arrival[nextProcess], burst[nextProcess], priority[nextProcess]));
